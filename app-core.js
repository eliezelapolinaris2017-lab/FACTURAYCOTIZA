...
/* Nota: el panel te muestra storageBucket con .firebasestorage.app,
pero para web debe ser .appspot.com */
const firebaseConfig = {
apiKey: "AIzaSyBM6gQ-BbqngwFF8rfLLVQnhQw-YSYHLAI",
...
client:"", lines:[], discountPct:0, taxPct:s.taxPercent, notes:"", status:"borrador",
...
<div style="font-size:12px">Número: ${num}<br>Fecha: ${new Date(d.date).toLocaleDateString(s.locale)}<br>Cliente: ${d.client||""}<br>${d.paymentMethod?`Pago: ${d.paymentMethod}${d.paymentRef?` — Ref: ${d.paymentRef}`:""}`:""}</div>
...
onAuthStateChanged(auth, (u)=>{ document.dispatchEvent(new CustomEvent("oasis-auth", { detail:{ user:u } })); });
});
}

/* Debug: ver en consola el proyecto activo */
console.log("Firebase app:", app.name, "| projectId:", app.options.projectId);
