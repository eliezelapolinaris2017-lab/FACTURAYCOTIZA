...
const Store = (() => {
  ...
  function setPINHash(pinHash) {
    const s = get("settings") || DEFAULTS.settings;
    s.pinHash = pinHash; set("settings", s);
  }
...
const Numbering = (() => {
  function next(tipo) {
    ...
    const n = s.counters[tipo] || 1;
    s.counters[tipo] = n + 1;
    Store.updateSettings({ counters: s.counters });
    return n;
  }
  function currentPrefix(tipo) {
    const s = Store.get("settings");
    return s.prefixes[tipo] || (tipo + "-");
  }
  function setCounter(tipo, num) {
    const s = Store.get("settings");
    s.counters[tipo] = Number(num)||1;
    Store.updateSettings({ counters: s.counters });
  }
  function setPrefix(tipo, pref) {
    const s = Store.get("settings");
    s.prefixes[tipo] = pref;
    Store.updateSettings({ prefixes: s.prefixes });
  }
...
const Docs = (() => {
  ...
  function reemitNumber(doc) {
    doc.prefix = Numbering.currentPrefix(doc.type);
    doc.number = Numbering.next(doc.type);
    return doc;
  }
...
<body>
  ...
  <table class="p-table">
    <thead>
      <tr><th>Ítem</th><th>Detalle</th><th style="text-align:right">Precio</th><th style="text-align:right">Cant.</th><th style="text-align:right">Importe</th></tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
  ...
  function reportHTML(list, rangeLabel) {
    ...
    return `<!doctype html>
<html lang="es"><head><meta charset="utf-8"><title>Reporte ${rangeLabel}</title>
<style>
  body{font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;margin:0;padding:10mm;}
  .p-head{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:10px}
  .p-head img{width:96px;height:96px;object-fit:contain;background:#fff;border:1px solid #ddd;border-radius:8px}
...
<body>
  ...
  <table class="p-table">
    <thead>
      <tr><th>Tipo</th><th>Número</th><th>Fecha</th><th>Cliente</th><th>Total</th><th>Estado</th></tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
  <script>window.onload = () => window.print();</script>
...
const Reports = (() => {
  ...
  function summarize(list) {
    ...
    list.forEach(d => {
      ...
      out.counts[d.type] = (out.counts[d.type]||0) + 1;
      out.totals[d.type] = (out.totals[d.type]||0) + t;
      out.counts.ALL++; out.totals.ALL += t;
    });
    return out;
  }
...
const UI = (() => {
  ...
  async function initAuth() {
    ...
    if (!s.pinHash) {
      ...
      els.authForm.onsubmit = async (e) => {
        ...
        const p2 = els.authPin2.value.trim();
  ...
  function updateDocButtons() {
    ...
    els.btnGuardarFinal.disabled = isAnnul;
  }
  ...
  function addLineaRow(line = { name:"", desc:"", price:0, qty:1 }) {
    ...
    const recalcLine = () => {
      const s = Store.get("settings");
      const imp = Utils.toNumber(priceEl.value) * Utils.toNumber(qtyEl.value||1);
      tr.querySelector(".l-imp").textContent = Utils.fmtMoney(imp, s.currency, s.locale);
      updateTotals();
    };
  ...
  els.btnQuickAdd.addEventListener("click", () => {
    const id = els.catalogQuickAdd.value;
    if (!id) return;
    const item = Store.getItems().find(i => i.id === id);
    if (!item) return;
    addLineaRow({ name: item.name, desc: item.desc||"", price: item.price||0, qty: 1 });
  });

  els.docForm.addEventListener("input", (e) => {
    if (["docDescuento","docImpuesto"].includes(e.target.id)) updateTotals();
  });

  els.btnGuardarBorrador.addEventListener("click", () => {
    currentDoc = collectDocFromForm();
    currentDoc.status = "borrador";
  ...
  els.btnReemitir.addEventListener("click", () => {
    if (!confirm("Esto asignará un nuevo número según el consecutivo actual. ¿Continuar?")) return;
    currentDoc = Docs.reemitNumber(collectDocFromForm());
    Store.saveDoc(currentDoc);
    els.docPrefijo.value = currentDoc.prefix;
    els.docNumero.value = currentDoc.number;
    renderHistorial();
  ...
  function renderHistorial() {
    ...
    els.histBody.querySelectorAll(".btn-ann").forEach(b => b.onclick = () => {
      const d = Store.getDocs().find(x => x.id === b.dataset.id);
      if (!confirm("¿Anular este documento?")) return;
      d.status = "anulado"; Store.saveDoc(d); renderHistorial();
    });
    els.histBody.querySelectorAll(".btn-pdf").forEach(b => b.onclick = () => {
      const d = Store.getDocs().find(x => x.id === b.dataset.id);
  ...
  function renderItems() {
    ...
    els.itemsBody.querySelectorAll(".i-del").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const items = Store.getItems().filter(i => i.id !== id);
        Store.setItems(items);
  ...
  function runReportRange() {
    ...
    const list = Reports.filterByRange(d, h);
    renderReportTable(list);
    return { list, label: labelFromRange(d, h) };
  }
  function runReportMonth() {
    const m = els.repMes.value;
    if (!m) { alert("Selecciona un mes."); return { list: [], label: "Mes (sin seleccionar)" }; }
    const list = Reports.filterByMonth(m);
    renderReportTable(list);
  ...
  function runReportCurrent() {
    ...
    els.repMes.value = ym;
    return runReportMonth();
  }
  function labelFromRange(d,h) {
    if (d && h) return `${Utils.formatDate(d, Store.get("settings").locale)} a ${Utils.formatDate(h, Store.get("settings").locale)}`;
    if (d) return `Desde ${Utils.formatDate(d)}`;
    if (h) return `Hasta ${Utils.formatDate(h)}`;
    return "Todo";
  }
  function labelFromMonth(m) {
    const [y,mm] = m.split("-");
    return `${mm}/${y}`;
  }
  ...
  function loadConfigForm() {
    ...
    if (s.logoDataUrl) { els.cfgLogoPreview.src = s.logoDataUrl; els.cfgLogoPreview.style.display="block"; }
    else { els.cfgLogoPreview.removeAttribute("src"); els.cfgLogoPreview.style.display="none"; }
    loadBrand();
  }
  ...
  els.btnGuardarConfig?.addEventListener("click", () => {
    const patch = {
      ...
      taxPercent: Utils.toNumber(els.cfgIVU.value),
      locale: els.cfgLocale.value.trim() || "es-PR",
    };
    ...
    loadBrand(); fillQuickAdd(); updateTotals();
  ...
  els.cfgLogo?.addEventListener("change", () => {
    ...
    reader.onload = () => {
      ...
      Store.updateSettings({ logoDataUrl: dataUrl });
      els.cfgLogoPreview.src = dataUrl; els.cfgLogoPreview.style.display="block";
      loadBrand();
  ...
  els.btnCambiarPIN?.addEventListener("click", async () => {
    ...
    if (!curr || !n1 || !n2) { els.cfgPinMsg.textContent = "Completa todos los campos."; return; }
    if (n1 !== n2) { els.cfgPinMsg.textContent = "El PIN nuevo no coincide."; return; }
    if (n1.length < 4) { els.cfgPinMsg.textContent = "El PIN debe tener al menos 4 dígitos."; return; }
    const s = Store.get("settings");
    const ok = (await Utils.hashPIN(curr)) === s.pinHash;
    if (!ok) { els.cfgPinMsg.textContent = "PIN actual incorrecto."; return; }
    const newHash = await Utils.hashPIN(n1);
    Store.setPINHash(newHash);
    els.cfgPinMsg.textContent = "PIN actualizado.";
